...
const SVG_UNITS_PER_MM = 96 / MM_PER_INCH;
...
let saxiSocket = null;
let saxiReconnectTimer = null;
let saxiAutoStreamTimer = null;
let saxiPendingStream = false;
...
const P = {
  ...
  blackLetterConnectedPath: false,
  presetMode: "none",
  presetInsetMM: 20,
  presetCols: 8,
  presetRows: 10,
  ...
  saxiHost: "127.0.0.1:9080",
  saxiAutoStream: false,
  saxiStatus: "disconnected",
};
...
function generateBlackLetterPaths(renderPoints, springSettings = P) {
  ...
  const strokes = centers.map((center) => [
    {
  ...
  if (!springSettings.blackLetterConnectedPath) {
    return strokes;
  }
  ...
  for (let i = 0; i < strokes.length; i += 1) {
    ...
    const ordered = i % 2 === 0 ? stroke : [stroke[1], stroke[0]];
    for (const point of ordered) {
      appendPointIfDistinct(connected, point);
    }
  }

  return connected.length >= 2 ? [connected] : [];
}
...
function buildPane() {
  ...
  const defaultSpringFolder = pane.addFolder({ title: "Default Spring Settings" });
  addSpringSettingsControls(defaultSpringFolder, P);

  const exportFolder = pane.addFolder({ title: "Export" });
  exportFolder.addInput(P, "svgFilename", { label: "Filename" });
  exportFolder.addInput(P, "saxiHost", { label: "Saxi Host" });
  exportFolder.addInput(P, "saxiAutoStream", { label: "Auto Saxi" });
  exportFolder.addMonitor(P, "saxiStatus", { label: "Saxi" });
  exportFolder.addButton({ title: "Connect Saxi" }).on("click", connectSaxi);
  exportFolder.addButton({ title: "Stream Spring" }).on("click", streamSpringToSaxi);
  exportFolder.addButton({ title: "Stream Spine" }).on("click", streamSpineToSaxi);
...
function hookUI() {
  ...
  document.getElementById("connectSaxiBtn").addEventListener("click", connectSaxi);
  document.getElementById("streamSaxiBtn").addEventListener("click", streamSpringToSaxi);
  document.getElementById("streamSpineSaxiBtn").addEventListener("click", streamSpineToSaxi);
...
function refreshAnchorMonitor() {
  ...
  addSpringSettingsControls(springFolder, activeStyle);
  springFolder.addButton({ title: "Use Defaults" }).on("click", () => {
...
function buildSaxiSpringSVG() {
  ensureGeometryCache();
  const svg = [];
  let pathCount = 0;
  const svgWidth = P.canvasWMM * SVG_UNITS_PER_MM;
  const svgHeight = P.canvasHMM * SVG_UNITS_PER_MM;

  svg.push('<?xml version="1.0" encoding="UTF-8"?>');
  svg.push(
    `<svg xmlns="http://www.w3.org/2000/svg" width="${fmt(svgWidth)}" height="${fmt(svgHeight)}" viewBox="0 0 ${fmt(svgWidth)} ${fmt(svgHeight)}">`
  );
  ...
  for (let segmentIndex = 0; segmentIndex < cachedSpringPaths.length; segmentIndex += 1) {
    ...
    for (const springPath of cachedSpringPaths[segmentIndex]) {
      ...
      pathCount += 1;
      svg.push(
        `<path d="${polylineToSaxiPath(springPath)}" fill="none" stroke="black" stroke-width="${fmt(Math.max(0.1, P.springStrokeMM * SVG_UNITS_PER_MM))}"/>`
      );
    }
  }

  svg.push("</svg>");
  return pathCount > 0 ? svg.join("\n") : "";
}

function buildSaxiSpineSVG() {
  ensureGeometryCache();
  const svg = [];
  let pathCount = 0;
  const svgWidth = P.canvasWMM * SVG_UNITS_PER_MM;
  ...
  for (let segmentIndex = 0; segmentIndex < cachedRenderSpinePaths.length; segmentIndex += 1) {
    ...
    pathCount += 1;
    svg.push(
      `<path d="${polylineToSaxiPath(renderPath)}" fill="none" stroke="black" stroke-width="${fmt(Math.max(0.1, P.spineStrokeMM * SVG_UNITS_PER_MM))}"/>`
    );
  }
...
function streamSpringToSaxi() {
  const svg = buildSaxiSpringSVG();
  if (!svg) {
    setSaxiStatus("no spring paths");
    return;
  }

  if (!saxiSocket || saxiSocket.readyState !== WebSocket.OPEN) {
    saxiPendingStream = true;
    connectSaxi();
    return;
  }

  saxiSocket.send(JSON.stringify({ c: "incoming-svg", p: { svg } }));
  setSaxiStatus(`streamed ${new Date().toLocaleTimeString()}`);
}

function streamSpineToSaxi() {
  const svg = buildSaxiSpineSVG();
  if (!svg) {
    setSaxiStatus("no spine paths");
    return;
  }
  ...
  saxiSocket.send(JSON.stringify({ c: "incoming-svg", p: { svg } }));
  setSaxiStatus(`spine streamed ${new Date().toLocaleTimeString()}`);
}

function queueSaxiAutoStream() {
  if (!P.saxiAutoStream) {
    return;
  }
  if (saxiAutoStreamTimer) {
    return;
  }
  saxiAutoStreamTimer = setTimeout(() => {
    saxiAutoStreamTimer = null;
    streamSpringToSaxi();
  }, 250);
}

function connectSaxi() {
  if (saxiSocket && saxiSocket.readyState === WebSocket.OPEN) {
    setSaxiStatus("connected");
    if (saxiPendingStream) {
      saxiPendingStream = false;
      streamSpringToSaxi();
    }
    return;
  }

  if (saxiSocket && saxiSocket.readyState === WebSocket.CONNECTING) {
    setSaxiStatus("connecting");
    return;
  }

  clearTimeout(saxiReconnectTimer);
  saxiReconnectTimer = null;
  const url = getSaxiWebSocketURL();
  setSaxiStatus(`connecting ${url}`);

  try {
    saxiSocket = new WebSocket(url);
  } catch (error) {
    setSaxiStatus(`connection failed: ${error.message}`);
    return;
  }

  saxiSocket.addEventListener("open", () => {
    setSaxiStatus("connected");
    if (saxiPendingStream) {
      saxiPendingStream = false;
      streamSpringToSaxi();
  ...
  saxiSocket.addEventListener("message", (event) => {
    handleSaxiMessage(event.data);
  });

  saxiSocket.addEventListener("close", () => {
    setSaxiStatus("disconnected");
    if (P.saxiAutoStream) {
      saxiReconnectTimer = setTimeout(connectSaxi, 1000);
    }
  });

  saxiSocket.addEventListener("error", () => {
    setSaxiStatus("socket error");
  });
}

function getSaxiWebSocketURL() {
  const host = String(P.saxiHost || "localhost:9080").trim();
  if (host.startsWith("ws://") || host.startsWith("wss://")) {
    return host.endsWith("/chat") ? host : `${host.replace(/\/$/, "")}/chat`;
  }
  return `ws://${host.replace(/^https?:\/\//, "").replace(/\/$/, "")}/chat`;
}

function handleSaxiMessage(data) {
  let msg;
  try {
    msg = JSON.parse(data);
  } catch (_error) {
  ...
  const nextW = Math.max(10, Number(msg.p.paperSize.x));
  const nextH = Math.max(10, Number(msg.p.paperSize.y));
  if (nearlyEqual(P.canvasWMM, nextW) && nearlyEqual(P.canvasHMM, nextH)) {
    return;
  }
  ...
  syncCanvasSize();
  redraw();
  setSaxiStatus(`paper ${fmt(nextW)} x ${fmt(nextH)} mm`);
}

function setSaxiStatus(nextStatus) {
  P.saxiStatus = nextStatus;
  const statusEl = document.getElementById("saxiStatus");
  if (statusEl) {
    statusEl.textContent = `Saxi: ${nextStatus}`;
  }
  if (pane) {
    pane.refresh();
  }
...
function createSpineStyle() {
  return {
    ...
    blackLetterConnectedPath: P.blackLetterConnectedPath,
    springArcRadiusMM: P.springArcRadiusMM,
    showSpring: P.showSpring,
  };
}

function addSpringSettingsControls(folder, target) {
  ...
  folder.addInput(target, "blackLetterConnectedPath", {
...
function applyDefaultSpringSettingsToStyle(style) {
  style.orbitMode = P.orbitMode;
  style.coilAmplitudeMM = P.coilAmplitudeMM;
  style.coilPitchMM = P.coilPitchMM;
  style.samplesPerTurn = P.samplesPerTurn;
  style.offsetLineCount = P.offsetLineCount;
  style.offsetGapMM = P.offsetGapMM;
  style.spineSmoothing = P.spineSmoothing;
  style.spineSampleStepMM = P.spineSampleStepMM;
  style.springArcRadiusMM = P.springArcRadiusMM;
  style.blackLetterAngleDeg = P.blackLetterAngleDeg;
  style.blackLetterNibWidthMM = P.blackLetterNibWidthMM;
  style.blackLetterConnectedPath = P.blackLetterConnectedPath;
  style.showSpring = P.showSpring;
}
...
function polylineToSaxiPath(points) {
  if (points.length === 0) {
    return "";
  }

  let d = `M ${fmt(points[0].x * SVG_UNITS_PER_MM)} ${fmt(points[0].y * SVG_UNITS_PER_MM)}`;
  for (let i = 1; i < points.length; i += 1) {
    d += ` L ${fmt(points[i].x * SVG_UNITS_PER_MM)} ${fmt(points[i].y * SVG_UNITS_PER_MM)}`;
  }
  return d;
}
